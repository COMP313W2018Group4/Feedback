...
module.exports.CreateNewfeedback = function(req, res, next) {
    ...
    feedback.create(newfeedback, (err, game) => {
      if (err) {
        console.error(err);
        res.end(err);
      } else {

        console.log("record committed to feedback collection")
        //use res.json to check and add to the datbase
        res.redirect('/');
      }
    });
